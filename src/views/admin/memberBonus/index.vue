...
<script>
import { getDrlBonus  , getDrlWeek , getBonusContent} from "@/api/member";
export default {
  name: "userCredit",
  components:{},
  data() {
  ...
  methods: {
    ...
    changeShow(row){
      ...
      this.DetDialog = true
      this.$refs.detail.getList(this.gametxnId,this.bonusCode,this.searchFrom.fromDate,this.searchFrom.toDate,this.nowGame)
    },
    changeDetDialog(val){
      this.DetDialog = val
    },
    ...
    getList(){
      ...
      if (this.searchFrom.fromDate && this.searchFrom.toDate) {
        ...
        const { fromDate, toDate , bonusCode } = this.searchFrom;
...
.content_box{
  ...
  padding: 10px;
  text-align: left;  line-height:25px; font-family:微软雅黑; font-size:medium;
}
</style>